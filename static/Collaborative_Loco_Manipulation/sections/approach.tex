...
The kinematics constraint in limits the possible foot EE movements inside a nominal box region to assure safety and avoid singularity.
% \begin{equation}
% \begin{aligned}
%     \nonumber
...
Foot placements are constrained to satisfy either a default terrain height or the elevation map. In perceptive mode, admissible footholds are restricted to convex safe regions extracted from the elevation map \cite{miki2022elevation}, ensuring collision-free and stable contacts. The convex regions are selected heuristically for each stance phase similar to \cite{grandia2023perceptive}.
...
\begin{subequations}\label{eq:updates}
    ...
    \end{align}
\end{subequations}
where for each subproblem, due to the nonlinear dynamics, we employ an Riccati-like Sequential Quadratic Programming (SQP) solver to efficiently get the reduced-size subproblem solutions. All inequality constraints are penalized with a relaxed log barrier. 
...
\begin{figure*}[htb]
    ...
    \vspace{-5mm}
    \caption{Illustration of diverse terrain navigation scenarios used to evaluate perceptive locomotion. \textbf{A.1--A.2}: Two robots with cargo across stepped terrain of uniform surface height, shown in rendered and RViz views. \textbf{B.1--B.2}: Two quadruped robots with cargo on a sloped surface, shown in rendered and RViz views. \textbf{C}: Two robots redirecting a box through a 90-degree turn in a narrow passage. \textbf{D}: Two robots with cargo following a circular path on an elevated annular platform. \textbf{E}: Three robots with cargo across stepped terrain of uniform surface height. \textbf{F}: Four robots with cargo on a sloped surface. \textbf{G}: Two robots with cargo performing obstacle avoidance on flat terrain.}
